...
public class SkillExecute extends TargettedSkill implements Listener {
    ...
    public class ExecuteBleedEffect extends PeriodicDamageEffect {
        
        public final double healReduct;
        public ExecuteBleedEffect(Skill skill, long period, long duration, double tickDamage, Player applier, double healReduct) {
            super(skill, "ExecuteBleedEffect", period, duration, tickDamage, applier);
            this.healReduct = healReduct;
        }
    ...
    public SkillResult use(Hero hero, LivingEntity target, String[] args) {
        ...
        double healReduct = SkillConfigManager.getUseSetting(hero, this, "heal-reduction-percent-base", 0.5, false)
                + SkillConfigManager.getUseSetting(hero, this, "heal-reduction-percent-level", 0.01, false) * hero.getLevel();
        if(healReduct > 1) {
            healReduct = 1;
        }
        if(Skill.damageCheck(hero.getPlayer(),target)) {
            ...
            ExecuteBleedEffect effect = new ExecuteBleedEffect(this, period, duration, damage, hero.getPlayer(), healReduct);
    ...
    public String getDescription(Hero hero) {
        ...
        return getDescription()
                ...
                .replace("$5", dF.format(healReduct * 100))
                .replace("$6", dF.format(cooldown * 0.001));
    }
    ...
    public ConfigurationSection getDefaultConfig() {
        ...
        node.set("heal-reduction-percent-base", 0.5);
        node.set("heal-reduction-percent-level", 0.01);
    ...
    public void onHeroRegainHealth(HeroRegainHealthEvent event) {
        if(event.getHero().hasEffect("ExecuteBleedEffect")) {
            double multiplier = 1 - ((ExecuteBleedEffect)event.getHero().getEffect("ExecuteBleedEffect")).healReduct;
